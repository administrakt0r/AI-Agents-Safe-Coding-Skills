...
This preflight now runs the deterministic `sync:release-state` flow, executes the local test suite, and performs `npm pack --dry-run --json` so release tags are validated against the same artifact path used later in CI.
...
That workflow now reruns `sync:release-state`, fails on canonical drift via `git diff --exit-code`, executes tests and docs security checks, and dry-runs the npm package before `npm publish`.
