...
- ðŸŽ¯ **Automatic transcript extraction** using `youtube-transcript-api`
- âœ… **Video validation** - Checks if video is accessible and has transcripts
- ðŸŒ **English-first transcript handling** - Prefers English captions when available
- ðŸ“Š **Comprehensive summaries** - Prioritizes detail and completeness
- ðŸ“ **Structured output** - Markdown with headers, sections, insights
- ðŸ” **Metadata included** - Video title, channel, duration, URL
- âš¡ **Error handling** - Clear messages for all failure scenarios
- ðŸ› ï¸ **Dependency management** - Offers to install requirements automatically
- ðŸ“Š **Progress gauge** - Visual processing tracker across all steps
- ðŸ’¾ **Flexible save options** - Summary-only, summary+transcript, or transcript-only (NEW v1.2.0)
...
âœ… Public videos with auto-generated captions  
âœ… Videos with manual subtitles/captions  
âœ… Videos with transcripts in any supported language

### Videos That Don't Work

âŒ Private or unlisted videos  
âŒ Videos with transcripts disabled  
âŒ Age-restricted videos (may require authentication)  
âŒ Videos without any captions/subtitles
...
```
âŒ No transcript available for this video
...
### Q: Can I summarize videos that were not originally published in English?

**A:** Yes. The skill prefers English captions when they exist, and otherwise works from the available transcript while keeping the generated summary in English.
...
Files are saved as `summary-{VIDEO_ID}-{YYYY-MM-DD}.md` (summary) or `transcript-{VIDEO_ID}-{YYYY-MM-DD}.txt` (transcript-only).
...
**Built with â¤ï¸ by Eric Andrade**
