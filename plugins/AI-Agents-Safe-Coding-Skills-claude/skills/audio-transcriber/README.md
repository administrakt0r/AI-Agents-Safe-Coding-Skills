...
- **ðŸ§  LLM Integration** - Claude CLI (primary) or GitHub Copilot CLI (fallback) for intelligent processing
...
- **ðŸ“Š Progress Indicators** - Visual progress bars (tqdm) and spinners (rich)
- **ðŸ“ Timestamp Filenames** - `transcript-YYYYMMDD-HHMMSS.md` + `ata-YYYYMMDD-HHMMSS.md`
- **ðŸ§¹ Auto-Cleanup** - Removes temporary `metadata.json` and `transcription.json`
- **ðŸŽ¨ Rich Terminal UI** - Beautiful formatted output with panels and colors

See **[CHANGELOG.md](./CHANGELOG.md)** for complete v1.1.0 details.

## ðŸŽ¯ Core Features

- **ðŸ“ Rich Markdown Output** - Structured reports with metadata tables, timestamps, and formatting
- **ðŸŽ™ï¸ Speaker Diarization** - Automatically identifies and labels different speakers
- **ðŸ“Š Technical Metadata** - Extracts file size, duration, language, processing time
- **ðŸ“‹ Intelligent Atas/Summaries** - Generated via LLM (Claude/Copilot) with customizable prompts
- **ðŸ’¡ Executive Summaries** - AI-generated structured summaries with topics, decisions, action items
- **ðŸŒ Multi-language** - Supports 99 languages with auto-detection
- **âš¡ Zero Configuration** - Auto-discovers Faster-Whisper/Whisper installation
- **ðŸ”’ Privacy-First** - 100% local Whisper processing, no cloud uploads
- **ðŸš€ Flexible Modes** - Transcript-only or intelligent processing with LLM
...
### Key Points
- ðŸ”¹ Dashboard redesign complete and staging-approved
- ðŸ”¹ API performance optimization prioritized
- ðŸ”¹ User testing scheduled for next week
...
A: Many common languages, including English, Spanish, French, German, Chinese, Japanese, and Arabic.
