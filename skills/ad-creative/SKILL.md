---
name: ad-creative
description: "When the user wants to generate, iterate, or scale ad creative for any paid advertising platform. Also use when the user mentions ad copy variations, ad creative, generate headlines, RSA headlines, bulk ad copy, ad iterations, creative testing, ad performance optimization, write ads."
metadata:
  version: 2.8.0
  risk: unknown
  source: https://github.com/coreyhaines31/marketingskills/tree/main/skills/ad-creative
---
...
## When to Use
Use this skill when the user wants to generate, iterate, or scale ad creative (headlines, descriptions, primary text, or full ad variations) for any paid advertising platform.
...
For recurring static ad production at volume (e.g., 50 concepts per batch), work from a **grounded inputs corpus** and the the static ad template library reference (not included). Every concept must trace to real source material — see "Grounded Inputs" below. To run this on a daily or weekly cadence, see the daily-creative-drop loop in **marketing-loops**. To present a batch for client or stakeholder approval, produce a the creative review page reference (not included).

### Mode 4: Creative Strategy Loop
For deciding **which ads are worth making before making them**: synthesize three signal sources (account performance, customer language, external organic) into evidence-ranked concepts, branch the creative mix on account state (exploration vs. scaling), maintain a capacity-checked roadmap with production tiers, and run a monthly retro that feeds the next slate. The full system lives in the references/creative-roadmap.md reference (not included); for hook generation and funnel-stage diagnosis inside any mode, load the references/hook-system.md reference (not included).
...
Per-concept format is defined in the references/static-ad-templates.md reference (not included). The human workflow this supports: open the folder, scan INDEX.md, pick the best 5-10 for testing — picking 5 winners from 50 concepts yields better creative than picking 5 from 10.

### Creative Review Page (client / stakeholder approval)

When a person who isn't you needs to review and pick — a client, a partner, a stakeholder — produce a **creative review page**: a self-contained HTML artifact that presents each concept as an in-feed platform mockup (Instagram/Facebook, with a whitelist-handle toggle), breaks carousels into a labeled frame-by-frame storyboard, lets them toggle headline/copy variations, and discloses what's grounded in real assets. It's the visual upgrade to INDEX.md — a decision made off one link instead of by reading markdown. The template ships at the assets/creative-review-template.html reference (not included) (one file, no build, hostable anywhere); populate its `DATA` object from your generated concepts. Full data model, grounding rules (the disclosure block is required), and delivery in the references/creative-review-page.md reference (not included).
...
For pulling performance data and managing campaigns, see the the tools registry reference (not included).

| Platform | Pull Performance Data | Manage Campaigns | Guide |
|----------|:---------------------:|:----------------:|-------|
| **Google Ads** | `google-ads campaigns list`, `google-ads reports get` | `google-ads campaigns create` | the google-ads.md reference (not included) |
| **Meta Ads** | `meta-ads insights get` | `meta-ads campaigns list` | the meta-ads.md reference (not included) |
| **LinkedIn Ads** | `linkedin-ads analytics get` | `linkedin-ads campaigns list` | the linkedin-ads.md reference (not included) |
| **TikTok Ads** | `tiktok-ads reports get` | `tiktok-ads campaigns list` | the tiktok-ads.md reference (not included) |
