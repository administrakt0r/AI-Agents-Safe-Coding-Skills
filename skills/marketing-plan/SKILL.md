---
name: marketing-plan
description: "When the user needs a comprehensive marketing plan for a client, a company they advise, or their own product. Generates an exhaustive 13-section plan structured by AARRR."
metadata:
  version: 2.0.0
  risk: safe
...
On invocation, the skill reads `~/marketing-plans/{client-slug}/progress.md` and resumes based on the state machine documented in the methodology documentation Step 1.1.2 (fresh → INIT → REVIEW → FINALIZE → finalized). Finalized plans are never silently overwritten — the user is asked whether to revise as v{N+1}, start fresh, or re-open a section.

## The three phases

The full workflow lives in the methodology documentation. Quick summary:
...
Use the embedded 17-section current-state rubric (the methodology documentation) as your scoring lens for Section 3 — score each section 0–5 against available materials.
...
Full template lives in the methodology documentation. The structure:
...
3. **Current state** — Team, budget, what's done, what's in-flight, what's stuck. Scored against the embedded 17-section current-state rubric (the methodology documentation).
...
Full primer in the methodology documentation. Quick rule:
...
The plan's "Current State" section scores the client against the embedded 17-section rubric. Full rubric in the methodology documentation — it's the source of truth, not a derivative of any external skill.
...
1. **`marketing-ideas`** — 139 proven marketing tactics. Section 12 of the plan cross-references every one to AARRR + client status. Detail in the methodology documentation.
...
The plan is **opinionated about which skills serve which stages.** Full mapping in the methodology documentation.
...
Full mapping in the methodology documentation.
...
Standard tiers in the methodology documentation:
...
The funding-stage anchors above tell you *what's in the ballpark*. To set the actual number defensibly, use one of two methods (full detail in the methodology documentation):
...
Pitch decks show hockey sticks. Real growth is a series of S-curves with plateaus between them. Full framework in the methodology documentation. Key implications for the plan:
...
Strategy lives in-house. Execution can — and often should — be outsourced. Full framework in the methodology documentation. Three implications for every plan:
...
Detail in the methodology documentation.
...
The full schema for `progress.md` and the resumption decision tree live in the methodology documentation Steps 1.1.1 and 1.1.2.
...
The full intake questionnaire lives in the methodology documentation. The most important questions:
