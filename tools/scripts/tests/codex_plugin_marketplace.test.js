...
const pluginEntry = marketplace.plugins.find((plugin) => plugin.name === "AI-Agents-Safe-Coding-Skills");
assert.ok(pluginEntry, "marketplace.json must include the AI-Agents-Safe-Coding-Skills plugin entry");
assert.deepStrictEqual(
  pluginEntry.source,
  {
    source: "local",
    path: "./plugins/AI-Agents-Safe-Coding-Skills",
  },
...
const pluginRoot = path.join(projectRoot, "plugins", "AI-Agents-Safe-Coding-Skills");
const pluginManifestPath = path.join(pluginRoot, ".codex-plugin", "plugin.json");
const pluginManifest = JSON.parse(fs.readFileSync(pluginManifestPath, "utf8"));

assert.strictEqual(pluginManifest.name, "AI-Agents-Safe-Coding-Skills");
