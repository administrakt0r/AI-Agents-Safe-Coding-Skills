...
text = """This PR updates the `azure-ai-ml-py` skill documentation to verify compatibility with SDK version 1.35.0. No breaking API changes were found since GA that would invalidate the existing code snippets. The maintenance ledger has been updated, and a dated run log has been provided.
...
heading_match = re.search(r'##\s+quality bar checklist\b', text, re.IGNORECASE)
print(heading_match)

if heading_match:
    section = re.split(r'\n##\s+', text[heading_match.start():], 2, flags=re.IGNORECASE)[0]
    print(bool(re.search(r'-\s+\[[xX]\]', section)))
    print(bool(re.search(r'-\s+\[\s\]', section)))
